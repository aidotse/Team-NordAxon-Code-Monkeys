...
root_path = "dataset/"
# data_loader
train_set = ExampleDataset("../../../../Dataset/train", transform=affine_augmentations())
test_set = ExampleDataset("../../../../Dataset/valid", transform=test_augmentations())
...
for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
    ...
    for iteration, (inputs, targets, masks) in enumerate(testing_data_loader, 1):
        ...
        input, target = inputs.float().to(device), targets[:,1].unsqueeze(1).float().to(device)

        prediction = net_g(input)

        mae = criterionL1(prediction, target)
        test_mae.append(mae)
